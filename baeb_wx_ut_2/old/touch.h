...
void touch(TPoint p, TEvent e)
{
  ...
  touch_last_x = map(p.y, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, tft.width());
  touch_last_y = map(p.x, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, tft.height());
#else
  touch_last_x = map(p.x, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, tft.width());
  touch_last_y = map(p.y, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, tft.height());
#endif
  switch (e)
  {
  case TEvent::Tap:
...
bool touch_touched()
{
  ...
  {
#if defined(TOUCH_SWAP_XY)
    touch_last_x = map(ts.points[0].y, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, tft.width() - 1);
    touch_last_y = map(ts.points[0].x, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, tft.height() - 1);
#else
